...
export function ReceiptDonorSearch({
  initialLabel = ""
}: {
  initialLabel?: string;
}) {
  ...
  return (
    <label className="full donor-lookup">
      ...
      />
      <div className="lookup-selected muted">Select a donor to load recent gifts and generate a receipt.</div>
          ...
          {results.map((option: DonorSearchOption) => (
            <button
              ...
              onClick={() => {
                ...
                router.push(`/receipts?donorId=${option.id}`);
              }}
            >
